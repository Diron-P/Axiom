...
static bool DecodeHeader(uint8_t* fileData, size_t file_size, DDSHeader** ddsHeader, DDSHeaderDX10** dxt10Header, uint8_t** ddsData)
{
	if (fileData == nullptr || ddsData == nullptr || file_size < sizeof(unsigned int) + sizeof(DDSHeader))
	{
		return false;
	}
	...
	*ddsData = fileData + header->size;
	...
	if (header->ddspf.flags & DDS_FORMAT_TYPE_FOURCC && header->ddspf.fourCC == MAKEFOURCC('D', 'X', '1', '0'))
	{
		...
		if (file_size < sizeof(unsigned int) + sizeof(DDSHeader) + sizeof(DDSHeaderDX10))
		{
			// Invalid texture
			return false;
		}

		*dxt10Header = (DDSHeaderDX10*)*ddsData;
		*ddsData += sizeof(DDSHeaderDX10);
	}

	return true;
}

inline static bool IsBitMask(const DDSPixelFormat& ddspf, const unsigned int rBitMask, const unsigned int gBitMask, const unsigned int bBitMask, const unsigned int aBitMask)
{
	return ddspf.rBitMask == rBitMask && ddspf.gBitMask == gBitMask && ddspf.bBitMask == bBitMask && ddspf.aBitMask == aBitMask;
}

static DXGI_FORMAT GetDXGIFormat(DDSPixelFormat& ddspf)
{
...
static void GetInitData(DDSHeader& header, uint32_t numTextures, SubResourceData* initData, uint8_t* ddsData, DXGI_FORMAT format)
{
	unsigned char blockSize = 0; // Block size in bytes.

	// Identify the block size for compressed texture formats that is necessary for computing the pitch.
	switch (format)
	{
	case DXGI_FORMAT_BC1_UNORM:
	case DXGI_FORMAT_BC1_UNORM_SRGB:
	case DXGI_FORMAT_BC1_TYPELESS:	 // ATI1
	...
	for (unsigned int i = 0; i < numTextures; ++i)
	{
		...
		for (unsigned int j = 0; j < header.mipMapCount; ++j)
		{
			...
			// Compute the width and height of each subsequent mip-map level.
			width >>= 1;
			height >>= 1;
			depth >>= 1;
...
Texture* ImageUtils::LoadDDSTextureFromFile(const char* fileName)
{
	...
	Axiom::Read(fileHandle, buffer, fileSize);
