...
namespace Math {
    ...
    inline Matrix4 Scale(const Matrix4& m, const Vector3& v)
    {
        Matrix4 scale(1.0f);
    ...
    inline Matrix4 Scale(const Matrix4& m, Vector3& v, float& s)
    {
        Normalize(v);
    ...
    // template <typename T>
    inline Matrix4 Rotate(const Matrix4& m, const float& angle, Vector3& v)
    {
        Normalize(v);
        ...
        Matrix4 result = rotate * m;
        result.SetTranslation(m.GetTranslation());
        return result;
    }

    inline Matrix4 Rotate(const Matrix4& m, const float& angle, const int& axis)
    {
        float cos = Cos(angle);
        float sin = Sin(angle);
    ...
    inline Matrix4 Translate(const Matrix4& m, const Vector3& v)
    {
        Matrix4 result(m);
    ...
    inline Matrix4 Reflect(const Matrix4& m, Vector3& v)
    {
        Normalize(v);
    ...
    inline Matrix4 LookAtLH(const Vector3& eye, const Vector3& target, Vector3& tmp)
    {
